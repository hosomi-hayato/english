...
$pdo=new PDO('mysql:host=localhost;dbname=english word;charset=utf8',
	...
	case 'insert':
 	...
 	case 'update':
		if (empty($_REQUEST['word']) || empty($_REQUEST['mean']) || empty($_REQUEST['example'])
                  || empty($_REQUEST['Japanese'])) break;
		$sql=$pdo->prepare(
			'update others_table  set mean=?, example=?, Japanese=?  where word=?');
		$sql->execute(
	...
	case 'delete':
		$sql=$pdo->prepare('delete from others_table where word=?');
...
foreach ($pdo->query('select * from others_table') as $row) {
	echo '<form class="ib" action="others-edit.php" method="post">';
...
?>
<form action="others-edit.php" method="post">
